...
impl MxlInstance {
    ...
    pub fn get_current_head_index(&self, rate: &mxl_sys::Rational) -> u64 {
        unsafe { self.context.api.mxl_get_current_head_index(rate) }
    }

    pub fn get_duration_until_head_index(
        &self,
        index: u64,
        rate: &mxl_sys::Rational,
    ) -> Result<std::time::Duration> {
        let duration_ns = unsafe { self.context.api.mxl_get_ns_until_head_index(index, rate) };
        if duration_ns == u64::MAX {
            Err(Error::Other(format!(
                "Failed to get duration until head index, invalid rate {}/{}.",
    ...
    pub fn timestamp_to_head_index(&self, timestamp: u64, rate: &mxl_sys::Rational) -> Result<u64> {
        let index = unsafe {
            self.context
                .api
                .mxl_timestamp_to_head_index(rate, timestamp)
        };
        if index == u64::MAX {
            Err(Error::Other(format!(
                "Failed to convert timestamp to head index, invalid rate {}/{}.",
                rate.numerator, rate.denominator
            )))
        } else {
            Ok(index)
    ...
    pub fn head_index_to_timestamp(&self, index: u64, rate: &mxl_sys::Rational) -> Result<u64> {
        let timestamp = unsafe { self.context.api.mxl_head_index_to_timestamp(rate, index) };
        if timestamp == u64::MAX {
            Err(Error::Other(format!(
                "Failed to convert head index to timestamp, invalid rate {}/{}.",
                rate.numerator, rate.denominator
            )))
        } else {
            Ok(timestamp)
