...
fn main() -> Result<(), mxl::Error> {
    ...
    let mut grain_index = mxl_instance.get_current_head_index(&grain_rate);
    info!(
    ...
    loop {
        ...
        let timestamp = mxl_instance.head_index_to_timestamp(grain_index + 1, &grain_rate)?;
        let sleep_duration =
            mxl_instance.get_duration_until_head_index(grain_index + 1, &grain_rate)?;
        info!(
